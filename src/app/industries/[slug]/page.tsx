...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${industry.title} Logistics Solutions | ${companyInfo.brandName}`,
    description: industry.description,
    alternates: {
      canonical: `${seoConfig.siteUrl}/industries/${industry.slug}`,
    },
    openGraph: {
      title: `${industry.title} Logistics | ${companyInfo.brandName}`,
      description: industry.description,
  ...
  return (
    <main>
      ...
      <section className="w-full py-10 md:py-14 bg-white">
        <div className="max-w-[1216px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#0052CC] mb-3">Industry overview</p>
      ...
      <section className="w-full py-10 md:py-14 bg-[#F8FAFC]">
        <div className="max-w-[1216px] mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#0052CC] mb-3">What we provide</p>
          <h2 className="text-xl sm:text-2xl font-bold text-[#0D1117] mb-8 tracking-[-0.02em]">Custom Solutions for {industry.title}</h2>
      ...
      <section className="w-full py-10 md:py-14 bg-white">
        <div className="max-w-[1216px] mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#0052CC] mb-3">Related services</p>
          <h2 className="text-xl sm:text-2xl font-bold text-[#0D1117] mb-6 tracking-[-0.02em]">Recommended Services</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {servicesData.slice(0, 3).map((s) => (
              <Link
                key={s.slug}
                href={`/services/${s.slug}`}
                className="group flex items-start gap-3 p-4 rounded-xl border border-[#E5E7EB] hover:border-[#0052CC]/25 hover:shadow-[0_4px_20px_rgba(0,82,204,0.08)] transition-all duration-200"
              >
                <span className="w-7 h-7 rounded-lg bg-[#EEF4FF] flex items-center justify-center shrink-0 mt-0.5">
                  <svg className="w-3.5 h-3.5 text-[#0052CC]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                  </svg>
                </span>
                <div>
                  <p className="text-sm font-semibold text-[#0D1117] group-hover:text-[#0052CC] transition-colors">{s.title}</p>
      ...
      <section className="w-full py-14 bg-[#0D1117]">
        <div className="max-w-[1216px] mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <Link
              href="/quote"
              className="inline-flex items-center justify-center px-6 py-2.5 text-sm font-semibold text-white bg-[#E53E3E] rounded-lg hover:bg-[#CC2A2A] transition-colors"
            >
              Request a Consultation
            </Link>
            <Link
