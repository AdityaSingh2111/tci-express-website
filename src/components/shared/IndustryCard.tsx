...
export function IndustryCard({ industry }: IndustryCardProps) {
  return (
    <Link
      ...
      className={
        'group relative flex flex-col justify-between h-full min-h-[160px] p-5 rounded-xl ' +
        'bg-white border border-[#E5E7EB] overflow-hidden ' +
    ...
    >
      ...
      <div className="w-8 h-8 rounded-lg bg-[#EEF4FF] flex items-center justify-center mb-3 transition-colors duration-200 group-hover:bg-[#DBEAFE]">
        <svg className="w-4 h-4 text-[#0052CC]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
          <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" />
        </svg>
      </div>

      <div>
        <h3 className="text-sm font-semibold text-[#0D1117] mb-1.5 group-hover:text-[#0052CC] transition-colors duration-200">
          {industry.title}
        </h3>
      ...
      {/* Arrow */}
      <div className="mt-4 flex items-center gap-1 text-[11px] font-semibold text-[#0052CC] opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        Explore
